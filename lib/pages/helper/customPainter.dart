...
class FacePainter extends CustomPainter {
 ...
 // final List<Rect>  rect;
  ...
  void paint(Canvas canvas, Size size) {
     state.loadImgage().then(((image)=>{
       img = image
      }));
    canvas.drawImage(img, Offset.zero, Paint());
    for(var i = 0; i< state.getfaceList.length ; i++){
      final rect = _scaleRect( rect: face[i].boundingBox,imageSize: size,widgetSize: size);
      //final eye = _scaleRect( rect: face[i].getLandmark(FaceLandmarkType.leftEye).position,imageSize: size,widgetSize: size);
      final paint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 5
      ..color = Colors.yellow;
     // canvas.drawCircle(rect.center, rect.height/2, paint);
      final lipPaint = Paint()..strokeWidth = 5.0..color = Colors.red;
      ...
      var bottomMouth = face[i].getLandmark(FaceLandmarkType.bottomMouth).position;
      var leftmouth = face[i].getLandmark(FaceLandmarkType.leftMouth).position;
      var rightmouth = face[i].getLandmark(FaceLandmarkType.rightMouth).position;
      var leftCheek = face[i].getLandmark(FaceLandmarkType.leftCheek).position;
      var rightcheek = face[i].getLandmark(FaceLandmarkType.rightCheek).position;
  ...
  void drawFaceMark(Canvas canvas ,Point<double> point,Size size , Paint paint){
  ...
  void drawSmile(Canvas canvas ,Rect rect ,Size size , Paint paint){
     ...
     final eyePaint = Paint()
      ...
      //? Smile
      canvas.drawArc(Rect.fromCenter(center: center, width: radius/2 ,height: radius/2) , 0 , math.pi, false , paint);
  }
